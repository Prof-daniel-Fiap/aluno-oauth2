...
class CursoResourceIT {
    ...
    void createCurso() throws Exception {
        ...
        restCursoMockMvc
            .perform(
                post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(curso))
            )
    ...
    void putWithMissingIdPathParamCurso() throws Exception {
        ...
        restCursoMockMvc
            .perform(
                put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(curso))
            )
