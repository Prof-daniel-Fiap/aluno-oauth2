...
export class TurmaService {
  ...
  protected convertDateFromClient(turma: ITurma): ITurma {
    return Object.assign({}, turma, {
      dataCriacao: turma.dataCriacao?.isValid() ? turma.dataCriacao.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((turma: ITurma) => {
        turma.dataCriacao = turma.dataCriacao ? dayjs(turma.dataCriacao) : undefined;
      });
    }
    return res;
  }
